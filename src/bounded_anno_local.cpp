...
void string_decomposer(vector<int> &optMotifs, vector<int> &optMotifStarts, vector<int> &optMotifEnds,
		       ...
		       SDTables &sdTables, vector<int > &mismatchCI)
{
  ...
  for (auto s=0; s < N; s++)
    {
	  ...
	  for (auto mi=0; mi < motifs[m].len; mi++)
		  ...
		  assert(m < sdTables.scoreMat.size());
		  assert(s+1 < sdTables.scoreMat[m].size());
		  assert(mi -1 < sdTables.scoreMat[m][s+1].size());
		...
		optPath=endPath;
	      ...
	      assert(m < sdTables.scoreMat.size());
	      assert(s+1 < sdTables.scoreMat[m].size());
	      assert(mi < sdTables.scoreMat[m][s+1].size());
      ...
      sdTables.scoreRow0[s+1] = maxScore - wraparoundPenalty;
  ...
  while (cs > 0 and cm >= 0 and endTrace == false)
    {
	  ...
	  if (arrow == left)
		  ...
		  assert(optMotifStarts.size() < 30000);
                }
	      cs--;
            }
        }
  ...
  assert(cmi < motifs[optMotifs[optMotifs.size()-1]].seq.size());
